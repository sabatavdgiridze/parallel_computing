...
struct BVH_node * create_BVH(struct particle_str * particles,int length) {  
  if (length == 1) {
    ...
    struct BVH_node * first_child = NULL;
    struct BVH_node * second_child = NULL;

    #pragma omp task shared(first_child)
    {
      first_child = create_BVH(particles, first_child_length);
    }

    #pragma omp task shared(second_child)
    {
      second_child = create_BVH(particles + first_child_length, length - first_child_length);
    }
...
void application(int N_array) {
    ...
    for (time = time_start; time <= time_finish; time++) {
        struct BVH_node * BVH_root;

        #pragma omp parallel
        {
          #pragma single nowait
          {
            BVH_root = create_BVH(particles, N_array);
          }
        }
        int idx;
        #pragma omp parallel
        {
          ...
          for (idx = 0; idx < N_array; idx++) {
              ...
              update_particles(particles, N_array, time_step);
          }
        }
        stack_pointer = 0;
    }
