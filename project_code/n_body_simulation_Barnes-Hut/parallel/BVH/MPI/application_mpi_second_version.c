...
application_mpi() {
    ...
    struct BVH_node_data * tree_buffer = (struct BVH_node_data *) malloc(sizeof(struct BVH_node_data) * 4 * max_block_size);
    int * next_buffer = (int *) malloc(sizeof(int) * 4 * max_block_size);


    struct particle_str * particles = (struct particle_str *) malloc(sizeof(struct particle_str) * processes_block_sizes[ID_process]);

    int * processes_block_sizes_send = (int *) malloc(sizeof(int) * N_processes);
    int * processes_block_displacements_send = (int *) malloc(sizeof(int) * N_processes);
    
    for (idx = 0; idx < N_processes; idx++) {
        processes_block_sizes_send[idx] = processes_block_sizes[idx] * 4;
        processes_block_displacements_send[idx] = processes_block_displacements[idx] * 4;
    }
    

    if (ID_process == root) {
        struct particle_str * particles_total = init_particles(N_array);
        double * particles_total_buffer = (double *) malloc(sizeof(double) * 4 * N_array);
        
        for (idx = 0; idx < N_array; idx++) {
            particles_total_buffer[4 * idx    ] = particles_total[idx].x_pos;
            particles_total_buffer[4 * idx + 1] = particles_total[idx].y_pos;
            particles_total_buffer[4 * idx + 2] = particles_total[idx].x_vel;
            particles_total_buffer[4 * idx + 3] = particles_total[idx].y_vel;
        }
        
        double * particle_buffer  = (double *) malloc(sizeof(double) * 4 * processes_block_sizes[ID_process]);
        MPI_Scatterv(particles_total_buffer, processes_block_sizes_send, processes_block_displacements_send, MPI_DOUBLE, particle_buffer, processes_block_sizes_send[ID_process], MPI_DOUBLE, root, MPI_COMM_WORLD);
        for (idx = 0; idx < processes_block_sizes[ID_process]; idx++) {
            particles[idx].x_pos = particle_buffer[4 * idx    ];
            particles[idx].y_pos = particle_buffer[4 * idx + 1];
            particles[idx].x_vel = particle_buffer[4 * idx + 2];
            particles[idx].y_vel = particle_buffer[4 * idx + 3];
            particles[idx].x_force = 0.0;
            particles[idx].y_force = 0.0;
            particles[idx].mass = 1.0;
        }
        free(particle_buffer);
        free(particles_total_buffer);
        free(particles_total);
    } else {
        double * particle_buffer  = (double *) malloc(sizeof(double) * 4 * processes_block_sizes[ID_process]);
        MPI_Scatterv(NULL, processes_block_sizes_send, processes_block_displacements_send, MPI_DOUBLE, particle_buffer, processes_block_sizes_send[ID_process], MPI_DOUBLE, root, MPI_COMM_WORLD);
        for (idx = 0; idx < processes_block_sizes[ID_process]; idx++) {
            particles[idx].x_pos = particle_buffer[4 * idx    ];
            particles[idx].y_pos = particle_buffer[4 * idx + 1];
            particles[idx].x_vel = particle_buffer[4 * idx + 2];
    ...
    struct BVH_node_data * flattened_tree = (struct BVH_node_data *) malloc(sizeof(struct BVH_node_data) * 4 * processes_block_sizes[ID_process]);
    int * next = (int *) malloc(sizeof(int) * 4 * processes_block_sizes[ID_process]);
    
    for (time = time_start; time <= time_finish; time+=time_step) {

        struct BVH_node * BVH_root = create_BVH(particles, processes_block_sizes[ID_process]);
        flatten_BVH_tree(BVH_root, flattened_tree, next);

        for (idx = 0; idx < processes_block_sizes[ID_process]; idx++) {
            compute_force_flattened_BVH_tree(&particles[idx], flattened_tree, next, gravity_constant, threshold);
        }

        int round;
        for (round = 0; round < N_processes; round++) {
            if (round == ID_process) {
                MPI_Bcast(next, 4 * processes_block_sizes[round], MPI_INT, round, MPI_COMM_WORLD);

                // send the tree 

                double * send_tree_buffer = (double *) malloc(sizeof(double) * 7 * 4 * processes_block_sizes[round]);
                for (idx = 0; idx < 4 * processes_block_sizes[round]; idx++) {
                    send_tree_buffer[7 * idx    ] = flattened_tree[idx].bounding_box.x_min;
                    send_tree_buffer[7 * idx + 1] = flattened_tree[idx].bounding_box.x_max;

                    send_tree_buffer[7 * idx + 2] = flattened_tree[idx].bounding_box.y_min;
                    send_tree_buffer[7 * idx + 3] = flattened_tree[idx].bounding_box.y_max;
                    
                    send_tree_buffer[7 * idx + 4] = flattened_tree[idx].x_center_of_mass;
                    send_tree_buffer[7 * idx + 5] = flattened_tree[idx].y_center_of_mass;
                    
                    send_tree_buffer[7 * idx + 6] = flattened_tree[idx].total_mass;
                }
                MPI_Bcast(send_tree_buffer, processes_block_sizes[round] * 7 * 4, MPI_DOUBLE, round, MPI_COMM_WORLD);
                free(send_tree_buffer);
            } else {
                for (idx = 0; idx < 4 * max_block_size; idx++) {
                    next_buffer[idx] = -1;
                }

                MPI_Bcast(next_buffer, 4 * processes_block_sizes[round], MPI_INT, round, MPI_COMM_WORLD);

                // receive the tree
                double * receive_tree_buffer = (double *) malloc(sizeof(double) * 4 * 7 * processes_block_sizes[round]);
                MPI_Bcast(receive_tree_buffer, processes_block_sizes[round] * 4 * 7, MPI_DOUBLE, round, MPI_COMM_WORLD);
                for (idx = 0; idx < 4 * processes_block_sizes[round]; idx++) {
                    tree_buffer[idx].bounding_box.x_min = receive_tree_buffer[7 * idx    ];
                    tree_buffer[idx].bounding_box.x_max = receive_tree_buffer[7 * idx + 1];

                    tree_buffer[idx].bounding_box.y_min = receive_tree_buffer[7 * idx + 2];
                    tree_buffer[idx].bounding_box.y_max = receive_tree_buffer[7 * idx + 3];
                    
                    tree_buffer[idx].x_center_of_mass = receive_tree_buffer[7 * idx + 4];
                    tree_buffer[idx].y_center_of_mass = receive_tree_buffer[7 * idx + 5];
                    
                    tree_buffer[idx].total_mass = receive_tree_buffer[7 * idx + 6];
                }
                
                for (idx = 0; idx < processes_block_sizes[ID_process]; idx++) {
                    compute_force_flattened_BVH_tree(&particles[idx], tree_buffer, next_buffer, gravity_constant, threshold);
                }

                free(receive_tree_buffer);
            }
        }
        update_particles(particles, processes_block_sizes[ID_process], time_step);
        stack_pointer = 0;
    }
